...
public interface IDiscoveryQueueService
{
    ...
    Task<bool> EnqueueAsync(CrawlSource source, string categoryKey, string url, string itemType, int depth, string? parentUrl, CancellationToken cancellationToken);
    Task<bool> EnqueueProductAsync(CrawlSource source, string categoryKey, string url, int depth, string? parentUrl, CancellationToken cancellationToken);
