...
{
    [HttpGet]
    public async Task<IActionResult> GetProducts([FromQuery] string? category, [FromQuery] string? search, [FromQuery] int? minSourceCount, [FromQuery] string? freshness, [FromQuery] string? conflictStatus, [FromQuery] string? completeness, [FromQuery] string? sort, [FromQuery] int page = 1, [FromQuery] int pageSize = 20, CancellationToken cancellationToken = default)
    {
        return Ok(await adminQueryService.ListProductsAsync(category, search, minSourceCount, freshness, conflictStatus, completeness, sort, page, pageSize, cancellationToken));
    }
