...
public sealed class LaptopAttributeNormaliserTests
{
    ...
    [Test]
    public void Normalise_MapsRepresentativeLaptopRetailerFixture()
    {
        var sut = new LaptopAttributeNormaliser();

        var result = sut.Normalise("laptop", new Dictionary<string, SourceAttributeValue>
        {
            ["Processor"] = new() { AttributeKey = "Processor", Value = "Intel Core Ultra 9 185H", ValueType = "string" },
            ["RAM Memory"] = new() { AttributeKey = "RAM Memory", Value = "32 GB", ValueType = "string" },
            ["SSD Capacity"] = new() { AttributeKey = "SSD Capacity", Value = "2 TB", ValueType = "string" },
            ["Drive Type"] = new() { AttributeKey = "Drive Type", Value = "solid state drive", ValueType = "string" },
            ["Screen Size"] = new() { AttributeKey = "Screen Size", Value = "40.64 cm", ValueType = "string" },
            ["Screen Resolution"] = new() { AttributeKey = "Screen Resolution", Value = "2560 x 1600", ValueType = "string" },
            ["OS"] = new() { AttributeKey = "OS", Value = "windows 11 pro", ValueType = "string" },
            ["Battery Life"] = new() { AttributeKey = "Battery Life", Value = "18 hr", ValueType = "string" },
            ["Weight"] = new() { AttributeKey = "Weight", Value = "1250 g", ValueType = "string" }
        });

        Assert.Multiple(() =>
        {
            Assert.That(result["cpu_model"].Value, Is.EqualTo("Intel Core Ultra 9 185H"));
            Assert.That(result["ram_gb"].Value, Is.EqualTo(32));
            Assert.That(result["storage_capacity_gb"].Value, Is.EqualTo(2048));
            Assert.That(result["storage_type"].Value, Is.EqualTo("SSD"));
            Assert.That(result["display_size_inch"].Value, Is.EqualTo(16m));
            Assert.That(result["native_resolution"].Value, Is.EqualTo("1600p"));
            Assert.That(result["operating_system"].Value, Is.EqualTo("Windows 11 Pro"));
            Assert.That(result["battery_life_hours"].Value, Is.EqualTo(18m));
            Assert.That(result["weight_kg"].Value, Is.EqualTo(1.25m));
        });
    }
}