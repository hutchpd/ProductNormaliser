...
public sealed class DataIntelligenceTests
{
    ...
    [Test]
    public async Task GetSourceQualityScoresAsync_UsesCategorySpecificCompletenessRubrics()
    {
        var context = MongoIntegrationTestFixture.Context;
        ...
        [
            ...
            {
                Id = "source-tv-1",
                SourceName = "multi-category-source",
                ...
                NormalisedAttributes = new Dictionary<string, NormalisedAttributeValue>
                {
                    ["energy_rating"] = CreateNormalisedAttribute("energy_rating", "E", "string", 0.95m),
                    ["total_capacity_litre"] = CreateNormalisedAttribute("total_capacity_litre", 400, "integer", 0.95m, "litre")
                }
            },
            new SourceProduct
            {
        ...
        var tvScores = await service.GetSourceQualityScoresAsync("tv", CancellationToken.None);
        var refrigeratorScores = await service.GetSourceQualityScoresAsync("refrigerator", CancellationToken.None);

        Assert.Multiple(() =>
        {
            Assert.That(tvScores, Has.Count.EqualTo(1));
            Assert.That(refrigeratorScores, Has.Count.EqualTo(1));
            Assert.That(tvScores[0].AverageMappedAttributes, Is.EqualTo(2.00m));
            Assert.That(refrigeratorScores[0].AverageMappedAttributes, Is.EqualTo(4.00m));
            Assert.That(tvScores[0].CoveragePercent, Is.EqualTo(33.33m).Within(0.01m));
            Assert.That(refrigeratorScores[0].CoveragePercent, Is.EqualTo(66.67m).Within(0.01m));
            Assert.That(refrigeratorScores[0].QualityScore, Is.GreaterThan(tvScores[0].QualityScore));
        });
    }
